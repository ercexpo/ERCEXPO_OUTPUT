...
pl = read_rename_csv(paste0("pl_survey_post.csv"), "_post") %>%
  ...
  mutate(news_self = rowMeans(
    select(., paste0("q8_h2_r", 1:9, "_w1")), na.rm = T))

us = read_rename_csv(paste0("us_survey_post.csv"), "_post") %>%
...
us = select(us, c(
  ...
  "domain_right_mean_w3_post", "news_self", "diet_lean_w2",
...
pl = select(pl, c(
  ...
  "domain_right_mean_w2_post", "news_self", "f1_pre", 
...
```{r}
# POLAND
pl$sex = recode(pl$sex, `1` = "Male", `2` = "Female")
```
...
```{r}
# UNITED STATES
us$condition = recode(us$condition, 
...
us$party_bin[us$party_bin == "Independent" & us$ideo < 5] = "Democrat" 
us$party_bin[us$party_bin == "Independent" & us$ideo > 5] = "Republican" 

# Impute using support for Trump
us$party_bin[us$party_bin == "Independent" & us$approve_trump < 5] = "Democrat" 
...
# Randomly impute remaining 17 independents
...
pl$att_str_pre = rowMeans(
  select(pl, paste0("att_str_", 1:4, "_pre")), na.rm = T)
...
# Exposure variables natural logarithm rescaled to 0 and 100
...
Save data
```{r}
write.csv(pl, paste0(datadir, "pl_survey_wide.csv"))
write.csv(us, paste0(datadir, "us_survey_wide.csv"))
write.csv(pl_long, paste0(datadir, "pl_survey_long.csv"))
write.csv(us_long, paste0(datadir, "us_survey_long.csv"))
```