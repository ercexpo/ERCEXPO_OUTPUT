...
df = read.csv(paste0(datadir, "us_survey_long_NEW.csv")) %>% 
  select(., vars) %>%
  mutate(country = "United States") %>% 
  bind_rows(
    list(., read.csv(paste0(datadir, "pl_survey_long_NEW.csv")) %>%
            select(., all_of(vars)) %>%
...
us_trace_visits <- read.csv(paste0(datadir, "us_trace_visits_NEW.csv")) 
us_trace_visits$created_local_date <- as.character(as.Date(us_trace_visits$created_local_date, format = "%Y-%m-%d"))

visits_sum_us = visits_sum_us %>% 
  left_join(
    aggregate(id ~ person_id + created_local_date, 
              data = us_trace_visits %>%
                filter(person_id %in% us_ids_exp$person_id), 
              FUN = length) %>% 
...
pl_trace_visits <- read.csv(paste0(datadir, "pl_trace_visits_NEW.csv")) 
pl_trace_visits$created_local_date <- as.character(as.Date(pl_trace_visits$created_local_date, format = "%Y-%m-%d"))

visits_sum_pl = visits_sum_pl %>% 
  left_join(
    aggregate(id ~ person_id + created_local_date, 
              data = pl_trace_visits %>%
                filter(person_id %in% pl_ids_exp$person_id), 
              FUN = length) %>% 
    setNames(., c("person_id", "date", "newscount")), 
    by = c("person_id", "date")
...
```{r}
## Heterogenous effects by party id and party strength
models_het = list()
...
for (var in vars[1:13]){
  ...
  for (cntry in c("Poland", "United States")){
    ...
    for (j in 1:2){
      ...
      eq = c(
      paste0(dv, " ~ condition + party_bin + party_bin*condition", iv), 
      paste0(dv, " ~ condition + party_strength + party_strength*condition", iv)
      )[j] 
      
      modelname = c("Partisanship",
                    "Partisan Identity Strength")[j]
      ...
      models_het[[length(models_het) + 1]] = lm(as.formula(eq), data = dat_new) %>%
        model_to_table() %>%
        mutate(model = paste0("Model ", j ,letters[i])) %>% 
        mutate(modelname = modelname) %>% 
        mutate(data = datname) %>% 
...
models_het = do.call(rbind, models_het)
